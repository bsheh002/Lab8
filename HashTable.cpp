...
*/
HashTable::HashTable (int s) { //constructor
	size = s;
...
*/
int HashTable::computeHash(const string &s) { //checks input string and uses it to determine size of hash
    int hashNum = 0;
    for(auto character = s.begin(); character!=s.end(); ++character){
        char ch = *character;
        hashNum = (hashNum*31+ch) % size;
    }
    return hashNum%size;
}
...
*/
void HashTable::put(const string &s, int score) { //adds to the score or creates an entry
	 int index = computeHash(s);
    for(auto entry = hashTable[index].begin(); entry!=hashTable[index].end();++entry){
        WordEntry &entryOfWords = *entry;
         if(entryOfWords.getWord() == s){
             entryOfWords.addNewAppearance(score);
             return;
         }
     }
...
double HashTable::getAverage(const string &s) { //gets the average of the wordEntry
    int index = computeHash(s);

    for(auto entry = hashTable[index].begin(); entry!=hashTable[index].end();++entry){ //loop that goes through the hash table
        WordEntry &entryOfWords = *entry;
        if(entryOfWords.getWord() == s){ //checks if word matches
            return entryOfWords.getAverage();
        }
    }
    return 2.0;
}
...
*/
bool HashTable::contains(const string &s) { //checks if word is in hsh table
    int index = computeHash(s);
    for(auto entry = hashTable[index].begin(); entry!=hashTable[index].end();++entry){
        WordEntry &entryOfWords = *entry;
        if(entryOfWords.getWord() == s){
...
int main() {
    ...
    if (myfile.fail()) { //checks if file opens
    ...
    while (!myfile.eof()) { //checks to see if there is still data coming through
    ...
    while(message.length() > 0) { //checks to see if message is empty
